...
`CANON@<unit-name>/` is the canonical decision record for this unit. It holds Architecture Decision Records (ADRs) and the templates and structure that govern them. CANON is part of every unit (root and sub-units alike) per §0014; the `REFERENCE@<root-unit>/` folder at the agency's root carries the procedural reference docs that explain how CANON works.
...
- **§-numbers are sequential and never reused.** Even rejected proposals consume a number. See the agency's `@{agency_dir}/REFERENCE@{agency_dir}/decision-process.md` for why.
...
| §0002 | [Use MADR + Why-statement as the ADR format](accepted/§0002-use-madr-with-why-statement.md) | accepted | {date} |
...
| §0014 | [Agency and unit structure; flat `@<unit>/` convention](accepted/§0014-agency-and-unit-structure.md) | accepted | {date} |
