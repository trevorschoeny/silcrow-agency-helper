...
- `CANON@{unit_name}/` — its decisions.
- `OPS@{unit_name}/` — its operational artifacts.
- Agent directories (`<role>@{unit_name}/`).
- Sub-units (if any), as siblings.
- `README.md`.

The internal structure is identical to any non-submodule unit per §0014; only the git relationship to the parent repo differs.
...
- `../../registrar@{unit_name}/AGENTS.md` — no special audit for submodule mode; normal unit↔ADR audit applies.
- `../../lead@{unit_name}/AGENTS.md` — Lead's git notes include submodule management.
