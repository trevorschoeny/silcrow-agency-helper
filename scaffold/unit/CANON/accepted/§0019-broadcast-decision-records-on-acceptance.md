...
- **Sub-unit awareness.** A Lead at the agency root authors an ADR that binds @product, @mobile, @platform — none of those Leads know unless they read the parent's `@<parent>/CANON@<parent>/accepted/` periodically.
...
A new `Kind: adr-acceptance-notice` for the message protocol (§0005). Operational details — filename, body skeleton, recipient-walk algorithm — live in the agency's `@<root-unit>/REFERENCE@<root-unit>/message-protocol.md`. The message is short, pointer-style: title, brief context, link to the ADR, supersession note if applicable.
...
- `../../REFERENCE@{unit_name}/message-protocol.md` — the operational details: filename convention, body skeleton, recipient-walk algorithm.
