...
- **Governance changes flow through canon.** Additions that affect `@{unit_name}/` tracking are governance decisions and need an ADR (see §0013 direction-of-constraint).
...
- **Positive:** `@{unit_name}/` contents are always tracked; audit history is never compromised by a stale `.gitignore`.
...
- **Ignoring contents of `@{unit_name}/`.** Violates the audit-history requirement. Every governance change must be committable.
