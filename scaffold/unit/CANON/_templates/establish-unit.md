...
- **Supersedes:** —
- **Superseded by:** —
- **Influences:** every subsequent operational artifact scoped to `@{unit_name}`; every ADR authored inside `@{unit_name}/CANON@{unit_name}/`.
...
- Every ancestor unit's ADRs bind `@{unit_name}` — `@{parent_unit_name}`'s `CANON@{parent_unit_name}/accepted/`, plus any further ancestors walking up to the agency's root.
- `@{unit_name}`'s own decisions live in `@{unit_name}/CANON@{unit_name}/accepted/`.
...
When this ADR is accepted, the following directory is created nested inside `@{parent_unit_name}/` (per §0014's flat layout):

```
{parent_path}/@{unit_name}/
├── CANON@{unit_name}/                      ← empty; @{unit_name}'s ADRs land here
│   ├── accepted/, proposed/, superseded/, rejected/, anti-patterns/, _templates/
│   └── README.md
├── OPS@{unit_name}/                        ← operational artifacts (open container)
├── README.md                               ← @{unit_name}'s overview, points up to the root's REFERENCE
├── {lead_dir}@{unit_name}/                 ← Lead agent (with inbox/archive/)
├── {implementer_dir}@{unit_name}/          ← Implementer agent (with inbox/archive/)
└── registrar@{unit_name}/                  ← Registrar agent (with inbox/archive/)
```

Sub-units have no `REFERENCE@<unit>/` of their own (§0014); foundational reference lives at the agency's root and is inherited by walking up. `@{unit_name}` inherits every ancestor unit's ADRs by reference — its own `CANON@{unit_name}/accepted/` starts empty and fills only with decisions specific to `@{unit_name}`.
...
- **Negative:** Additional governance overhead (another Registrar, another set of inboxes, another `CANON@<unit-name>/`).
...
- `@{parent_unit_name}`'s `@{parent_unit_name}/README.md` — governance overview of the parent unit.
...
to the parent's CANON@<parent-unit-name>/accepted/ with the next available §-number.
