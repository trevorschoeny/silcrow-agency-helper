...
When in doubt, write the decision. A minimal ADR (see `../CANON@{unit_name}/_templates/madr-minimal.md`) takes five minutes. The cost of losing a rationale you later need is much higher.
...
- **Unique within the unit's record** — no per-topic sub-numbering. Per §0014, every unit (including the root) is its own §-numbering scope: each unit's `@{unit_name}/CANON@{unit_name}/` carries its own monotonic sequence, and there is no separate registry above or beside the units that assigns §-numbers across the tree.
...
The §-numbering discipline above governs ADR *identity* and is scaffold-prescribed. How ADRs are *organized* within `accepted/` — whether to introduce topic subfolders — is a **local decision**. See `../CANON@{unit_name}/README.md` and the "Partitioning at scale" section of `../{registrar_dir}@{unit_name}/AGENTS.md` for the mechanical procedure.
...
```
┌────────┐  draft  ┌─────────────────┐  commit  ┌──────────────────────────────┐
│ author │ ───────▶│ author's dir    │ ────────▶│ @{unit_name}/CANON@{unit_name}/accepted/│
...
└──────────────┘         └──────────┘         └───────────┘           └──────────┘
                                                    ...
                                                    │ rejected
                                                    ▼
                                            @{unit_name}/CANON@{unit_name}/rejected/
```
...
**Standalone** — as a record in `@{unit_name}/CANON@{unit_name}/anti-patterns/`, using the `ap-NNN` numbering sequence. Appropriate when the anti-pattern is reusable.
...
When the {lead_role} or {user_role} adds a new unit via the `silcrow:silcrow-add-unit` skill, the skill authors a unit-establishing ADR automatically (§0014). The ADR lands in the parent's `@<parent-unit-name>/CANON@<parent-unit-name>/accepted/` with the establishing reasoning captured. This is the fast path for a specific kind of structural decision; the skill is doing what the author would otherwise do by hand.
...
The {lead_role} drafts in `@{unit_name}/{lead_dir}@{unit_name}/draft-logging-adr.md`, assigns the next §-number by checking `accepted/` (e.g. §0019), moves the file to `@{unit_name}/CANON@{unit_name}/accepted/§0019-use-structured-logging.md`, sets `Status: accepted`, and commits `§0019: use structured logging`. Either Lead or Registrar updates the index — the Registrar will fix it at next audit if the Lead doesn't.

### Implementer draft with approval (§0012)

The {implementer_role} drafts in their own directory, moves the draft to `@{unit_name}/CANON@{unit_name}/proposed/draft-correlation-id.md`, and messages the {lead_role}: *"Drafted ADR proposing correlation IDs in API error responses — context: repeated debugging difficulty tracing errors across services. Please review."* Lead responds: accept (move to `accepted/` with §-number), request revisions (stays in `proposed/`), or reject (Registrar moves to `rejected/` with §-number — rejections consume §-numbers too).
...
- **Standalone anti-pattern** in `@{unit_name}/CANON@{unit_name}/anti-patterns/ap-007-...md` — reusable knowledge for future cache discussions.
...
- `../{registrar_dir}@{unit_name}/AGENTS.md` — the Registrar's side of every operation above.
...
- `../CANON@{unit_name}/_templates/` — the templates you'll use.
- `../CANON@{unit_name}/accepted/§0011-registrar-as-async-auditor.md` — why Lead commits direct.
- `../CANON@{unit_name}/accepted/§0012-user-as-principal-and-local-tier-numbering.md` — the Implementer-draft-with-approval path.
- `../CANON@{unit_name}/accepted/§0013-canonical-and-operational-artifacts.md` — the promotion rule and reference rule.
- `../CANON@{unit_name}/accepted/§0015-update-audits-produce-audit-adrs.md` — the `:silcrow-update` audit-ADR pattern.
