...
Read this file **when you're invoked to audit the record** (by User, Lead, or via the `:silcrow-update` skill). Don't load it preemptively in other sessions — the core audit stance and categories are in `../{registrar_dir}@{unit_name}/AGENTS.md`; this file is the detailed walk-through for active audit sessions.

---

## A. Form

- [ ] Every accepted ADR uses one of the templates in `@{unit_name}/CANON@{unit_name}/_templates/`.
...
- [ ] Every `@<unit-name>/` directory has a registering ADR in the parent's `@<parent-unit-name>/CANON@<parent-unit-name>/accepted/`.
