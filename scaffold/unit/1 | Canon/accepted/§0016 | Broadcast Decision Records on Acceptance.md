...
- **Supersedes:** —
- **Superseded by:** —
- **Influences:** every ADR acceptance event from this point forward; the Registrar's audit-ADR workflow (§0013); the Implementer's draft-with-approval flow (§0010); every agent's awareness of binding decisions.
- **Influenced by:** §0001, §0005, §0009, §0010, §0012, §0013
...
and neglected putting notices in agent instruction files (AGENTS.md edits), centralizing broadcasts on the Registrar (which would re-introduce sync gatekeeping that §0009's audit pattern eliminates), and leaving agents to check the record on demand (which produces the very awareness drift this ADR closes),
...
§0009 establishes the Registrar as an async auditor — they do not gatekeep every commit before it lands in `accepted/`. Lead direct-commits per §0009's first-class authority. Implementer drafts go through Lead approval per §0010. The User authors directly per their principal role. Audit ADRs are authored by the Registrar per §0013.
...
- **Don't re-bottleneck the Registrar.** §0009 deliberately keeps the Registrar out of every-commit gating. Centralizing broadcasts on Registrar would back-door that gatekeeping.
- **Inheritance shapes recipient set.** ADRs at unit X bind X + descendants (§0012). Broadcast scope mirrors authority scope.
...
2. **Registrar broadcasts on every acceptance.** Rejected: re-introduces sync gatekeeping that §0009's audit pattern eliminates. Registrar would have to be in the loop for every commit.
3. **AGENTS.md notice (transient appended block, deleted after agent reads).** Rejected: introduces a special-case mechanism for what the inbox channel already does, and editing instruction files for transient state cuts against the canon/operational discipline (§0011).
...
|---|---|---|
| Lead direct-commits to `accepted/` (§0009) | Lead | Lead |
...
| Registrar authors an audit ADR (§0013) | Registrar | Registrar |
| `:silcrow-add-unit` writes an establishing ADR | Lead or User (whoever ran the skill) | The same Lead or User |
| Registrar makes a procedural correction (§0009 hybrid authority) | n/a — the original author stands | No broadcast — procedural corrections aren't acceptance events |
...
ADRs propagate downward along inheritance edges (§0012), not up or sideways. Don't broadcast to ancestor units, peer units, or cousin units — they aren't bound by this ADR.
...
When the Lead broadcasts an ADR drafted by the Implementer, the broadcast names both: *"drafted by {implementer_role} @ {unit_name}, approved by {lead_role} @ {unit_name}."* This honors the §0010 authoring split (the writing is the Implementer's; the authority is the Lead's).
...
- **Registrar broadcasting on the author's behalf as a routine.** That re-creates sync gatekeeping. The Registrar broadcasts only their own audit ADRs (§0013) and only as a courtesy if asked for User-authored ADRs.
...
- Cross-agency interaction patterns produce broadcast needs the current downward-only rule doesn't cover (note: cross-agency interaction is collaborative-not-hierarchical per §0012; broadcasts inside one agency stay scoped to that agency's tree regardless).

## References

- `§0005` — inter-agent communication via inboxes; the channel this ADR uses.
- `§0009` — Registrar as async auditor; the principle this ADR preserves by making authors broadcast.
