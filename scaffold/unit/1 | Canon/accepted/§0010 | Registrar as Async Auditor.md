...
On-demand audits cover form, contradictions, staleness, citation integrity, orphans, scope violations (§0018), federation-rule violations, unsafe references (§0012), unit↔ADR consistency (§0013), and git hygiene (§0016, informational). The full checklist lives in `../../Registrar @ {unit_name}/AGENTS.md`.
...
The `proposed/` directory is retained as a **voluntary pre-review channel**: Lead may commit directly to `accepted/` or drop into `proposed/` for Registrar eyes first; Implementers use `proposed/` as required staging under §0011's draft-with-approval path.
...
- §0011 — Implementer drafting-with-approval path uses `proposed/` as required staging.
- §0012 — unsafe-reference audit item.
- §0013 — unit↔ADR consistency audit item.
- §0014 — `:silcrow-update` audit-ADR pattern builds on this model.
- §0016 — governance commit convention; Registrar audits for unpushed governance.
