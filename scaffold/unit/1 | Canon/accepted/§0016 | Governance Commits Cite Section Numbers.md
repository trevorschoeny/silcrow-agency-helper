...
- **Influenced by:** §0001, §0003, §0012, §0015
...
we decided for **a two-tier convention split along the canon/operational line** — governance-touching commits cite the governing §NNNN (`§0012: add canon/operational foundation doc` or `Update Registrar audit checklist (per §0010)`); operational commits are free-form —
...
to achieve `git log --grep='§0012'` as "everything that happened because of §0012" while letting operational commits match whatever style the work calls for,
...
because the canon/operational split (§0012) implies different disciplines for different content, and reliable §NNNN traceability makes the governance record navigable in a way no amount of commit-log prose can replicate.

## Context and problem statement

The scaffold uses git for version control (per §0015's `.gitignore` decision) and treats the decision record as load-bearing. Those two facts interact at the commit-message level:

- A commit that adds `§0012` to `@ {unit_name}/1 | Canon/accepted/` is a governance event; future agents will want to search for "what happened with §0012?" and find every commit that implemented it.
...
- **Follow the canon/operational line.** The canonical/operational split (§0012) is the scaffold's organizing principle; commit discipline should respect it.
...
- **Governance-touching commits** — any commit that creates, accepts, supersedes, or edits an ADR, agent instruction, foundation doc, or governance README — use the form `§NNNN: <short imperative>`.
  - When the commit *is* the ADR itself (e.g., creating §0012): `§0012: canon/operational split and reference rule`.
  ...
  - When a commit touches multiple ADRs: list them. Example: `§0010, §0011: registrar and tier refinements`.
...
- **Update audits** — the `:silcrow-update` skill's one-commit-per-invocation uses the form `§NNNN: update audit — accepted A, rejected B, deferred C` (with the audit ADR's §-number; see §0014).
...
The convention is documented (here) and followed by habit. The Registrar's advisory git role (§0010) may flag commits that appear to violate the convention, but only as informational — never as blocking.
...
- **Positive:** `git log --grep='§0012'` reliably surfaces all §0012-related commits.
...
- **Positive:** Aligns with the canon/operational split (§0012).
...
- **Missing citation on a supersession.** Superseding §0008 with §0010 is a governance-touching commit; the message must cite §0010 (creating) and §0008 (superseding). Without the citation, the supersession is invisible to `git log --grep='§0008'`.
...
- §0010 — Registrar as async auditor; git-hygiene items are advisory.
- §0012 — canon/operational split that this convention mirrors.
- §0014 — `:silcrow-update` audit commits use this convention.
- §0015 — `.gitignore` defaults; this ADR completes the initial git-convention set.
