...
`1 | Canon/` is the canonical decision record for this unit. It holds Architecture Decision Records (ADRs) and the templates and structure that govern them. `1 | Canon/` is part of every unit (root and sub-units alike) per §0013; the `3 | Silcrow Agency Reference/` folder at the agency's root carries the procedural reference docs that explain how this folder works.
...
The `proposed/` directory is voluntary for Leads (who may commit directly to `accepted/` per §0010) and required for Implementers (who draft into `proposed/` awaiting approval per §0011).
...
| §0009 | [Roster change protocol](accepted/§0009%20%7C%20Roster%20Change%20Protocol.md) | accepted | {date} |
| §0010 | [Registrar operates as async auditor, not sync gatekeeper](accepted/§0010%20%7C%20Registrar%20as%20Async%20Auditor.md) | accepted | {date} |
| §0011 | [User as principal; local tier numbering; Implementer drafts-with-approval](accepted/§0011%20%7C%20User%20as%20Principal%20and%20Local%20Tier%20Numbering.md) | accepted | {date} |
| §0012 | [Canonical and operational artifacts: direction of constraint, promotion rule, reference rule](accepted/§0012%20%7C%20Canonical%20and%20Operational%20Artifacts.md) | accepted | {date} |
| §0013 | [Agency and unit structure; flat `@ <Unit>/` convention](accepted/§0013%20%7C%20Agency%20and%20Unit%20Structure.md) | accepted | {date} |
| §0014 | [Update audits produce per-session audit ADRs](accepted/§0014%20%7C%20Update%20Audits%20Produce%20Audit%20ADRs.md) | accepted | {date} |
| §0015 | [Agency default `.gitignore` — OS, editor, and secrets patterns only](accepted/§0015%20%7C%20Default%20Gitignore.md) | accepted | {date} |
| §0016 | [Governance commits cite the governing §NNNN; operational commits are free-form](accepted/§0016%20%7C%20Governance%20Commits%20Cite%20Section%20Numbers.md) | accepted | {date} |
| §0017 | [Decision-record acceptance is broadcast to every bound agent](accepted/§0017%20%7C%20Broadcast%20Decision%20Records%20on%20Acceptance.md) | accepted | {date} |
| §0018 | [Agency scope](accepted/§0018%20%7C%20Agency%20Scope.md) — *seed, expected to be superseded early; intentional first lesson in the supersession discipline* | accepted | {date} |

**About the founding set.** §0001 records the decision to adopt this scaffold. §0002–§0017 are **constitutional decisions inherited from the scaffold** via §0001 — they capture the load-bearing choices that define how this agency runs. Each names its alternatives and cites its foundations so it can be evaluated or superseded on its own merits, like any other ADR.

- §0018 (Agency Scope) is explicitly a seed — the scaffold invites the User and Lead to supersede it with a richer scope statement as an early collaborative task. Its supersession is the first concrete demonstration of §0017 (broadcast on acceptance).
- §0009–§0017 evolve earlier decisions or extend them: §0010 supersedes §0008 (Registrar operating mode); §0011 extends §0006 (tier model + authorship); §0012 establishes the canon/operational split that the rest of the agency's work flows within; §0013 codifies the flat `@ <Unit>/` layout with numeric-prefix governance folders (`1 | Canon`, `2 | Working Files`, `3 | Silcrow Agency Reference`); §0014–§0016 cover update audits, gitignore defaults, and commit conventions; §0017 establishes that ADRs broadcast through inboxes on acceptance, propagating awareness along the inheritance edges of the unit tree.

## Superseded decisions

| § | Title | Superseded by | Date |
|---|---|---|---|
| §0008 | [Registrar authority is procedural, not substantive](superseded/§0008%20%7C%20Registrar%20Procedural%20Authority.md) | §0010 | {date} |
