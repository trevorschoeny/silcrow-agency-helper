...
- **Influenced by:** §0008 (roster change protocol), §0010 (tier model), §0012 (agency and unit structure), §0017 (honest minimalism), §0018 (agency scope).
...
considered in the conversation that led to adding this agent. Don't
inherit suggested rejection reasons as boilerplate — fill them from the
real conversation, or write a single honest sentence (e.g., "No
alternatives were seriously considered; the need for a dedicated agent
was self-evident from the conversation.") and skip the numbered list.
-->
...
2. **Distribute the work across existing agents in `@ {unit_name}`.** {Rejected because — fill from the actual conversation, or omit if not genuinely considered.}
3. **Defer.** {Rejected because — fill from the actual conversation, or omit if not genuinely considered.}
...
- `§0018` — agency scope (the new agent operates within agency scope and `@ {unit_name}`'s scope).
