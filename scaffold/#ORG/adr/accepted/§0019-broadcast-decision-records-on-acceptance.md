...
- **Supersedes:** —
- **Superseded by:** —
- **Influences:** every ADR acceptance event from this point forward; the Registrar's audit-ADR workflow (§0015); the Implementer's draft-with-approval flow (§0012); every agent's awareness of binding decisions.
- **Influenced by:** §0001, §0005, §0011, §0012, §0014, §0015
...
and neglected putting notices in agent instruction files (AGENTS.md edits), centralizing broadcasts on the Registrar (which would re-introduce sync gatekeeping the §0011 audit pattern moved away from), and leaving agents to check the record on demand (which produces the very awareness drift this ADR closes),
...
§0011 established the Registrar as an async auditor — they no longer gatekeep every commit before it lands in `accepted/`. Lead direct-commits per §0011's first-class authority. Implementer drafts go through Lead approval per §0012. The User authors directly per their principal role. Audit ADRs are authored by the Registrar per §0015.
...
- **Sub-unit awareness.** A Lead at the agency root authors an ADR that binds @product, @mobile, @platform — none of those Leads know unless they read the parent's `#ORG@<parent>/adr/accepted/` periodically.
...
- **Don't re-bottleneck the Registrar.** §0011 deliberately moved Registrar out of every-commit. Centralizing broadcasts on Registrar would back-door that gatekeeping.
- **Inheritance shapes recipient set.** ADRs at unit X bind X + descendants (§0014). Broadcast scope mirrors authority scope.
...
2. **Registrar broadcasts on every acceptance.** Rejected: re-introduces sync gatekeeping the §0011 audit pattern moved away from. Registrar would have to be in the loop for every commit.
3. **AGENTS.md notice (transient appended block, deleted after agent reads).** Rejected: introduces a special-case mechanism for what the inbox channel already does, and editing instruction files for transient state cuts against the canon/operational discipline (§0013).
...
|---|---|---|
| Lead direct-commits to `accepted/` (§0011) | Lead | Lead |
...
| Registrar authors an audit ADR (§0015) | Registrar | Registrar |
| `:silcrow-add-unit` writes an establishing ADR | Lead or User (whoever ran the skill) | The same Lead or User |
| Registrar makes a procedural correction (§0011 hybrid authority) | n/a — the original author stands | No broadcast — procedural corrections aren't acceptance events |
| ADR moved to `rejected/` | n/a | No broadcast — existing acknowledgment to drafter is sufficient |

### Recipient set

Walk down the tree from the accepting unit. For every unit at or below (sub-units, sub-sub-units, recursively), drop the message in every agent's inbox at `#ORG@<unit>/agents/<role>@<unit>/inbox/`. Skip the broadcaster themselves.

ADRs propagate downward along inheritance edges (§0014), not up or sideways. Don't broadcast to ancestor units, peer units, or cousin units — they aren't bound by this ADR.

### Message format

A new `Kind: adr-acceptance-notice` for the message protocol (§0005). Operational details — filename, body skeleton, recipient-walk algorithm — live in the agency's `#ORG@<root-unit>/docs/message-protocol.md`. The message is short, pointer-style: title, brief context, link to the ADR, supersession note if applicable.

### Implementer-drafted ADRs name both author and approver

When the Lead broadcasts an ADR drafted by the Implementer, the broadcast names both: *"drafted by {implementer_role} @ {unit_display}, approved by {lead_role} @ {unit_display}."* This honors the §0012 authoring split (the writing is the Implementer's; the authority is the Lead's).

### Self-bootstrapping

This ADR establishes the rule that ADRs broadcast on acceptance. Agent template instructions executed at scaffold init carry the rule from Day 1; the rule isn't dependent on §0019's own acceptance to take effect. §0019 is the canonical record of the rule; the AGENTS.md instructions are the operational expression. The first ADR an agency authors after init (typically the §0020 supersession that replaces the agency-scope seed) is the first concrete demonstration of the broadcast.
...
- **Positive:** Inheritance scope (§0014) and broadcast scope are the same — the rule is intuitive: "decisions go where they bind."
- **Positive:** The §0020 supersession ceremony — the agency's first ADR action — broadcasts naturally, reinforcing the discipline by demonstration.
...
- **Registrar broadcasting on the author's behalf as a routine.** That re-creates sync gatekeeping. The Registrar broadcasts only their own audit ADRs (§0015) and only as a courtesy if asked for User-authored ADRs.
...
- Cross-agency interaction patterns produce broadcast needs the current downward-only rule doesn't cover (note: cross-agency interaction is collaborative-not-hierarchical per §0014; broadcasts inside one agency stay scoped to that agency's tree regardless).

## References

- `§0005` — inter-agent communication via inboxes; the channel this ADR uses.
- `§0011` — Registrar as async auditor; the principle this ADR preserves by making authors broadcast.
- `§0012` — User as principal; local tier numbering; Implementer drafts-with-approval; defines the authoring split that determines who broadcasts.
- `§0014` — agency and unit structure; defines the inheritance edges along which broadcasts propagate.
- `§0015` — update audits produce per-session audit ADRs; audit ADRs broadcast like any other ADR per this rule.
- `§0020` — agency scope (seed); the supersession that replaces it is the first concrete broadcast.
- `../../docs/message-protocol.md` — the operational details: filename convention, body skeleton, recipient-walk algorithm.
