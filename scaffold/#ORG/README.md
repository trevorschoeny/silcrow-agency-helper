...
- **New units** — use the `agent-org-scaffold:add-unit` skill, which authors the establishing ADR and creates the unit's `#ORG/` and directory structure in one motion (per §0015).
- **Scaffold updates** (new ADRs, reorg, conventions shipped by the plugin) — use the `agent-org-scaffold:update` skill. The Registrar orchestrates; every change is user-approved; the session produces one audit ADR (§0016).
