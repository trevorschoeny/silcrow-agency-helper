...
### Y-statements (Zimmermann, 2012)

**Olaf Zimmermann** introduced *Y-statements* in his SATURN 2012 talk "Making Architectural Knowledge Sustainable: Industrial Practice Report and Outlook." The full treatment appears in his 2020 essay *Architectural Decisions — The Making Of* (ozimmer.ch/practices/2020/04/27/ArchitectureDecisionMaking.html).

A Y-statement is the compressed one-sentence form of a decision:
...
The genius of the format is that if you can write a coherent Y-statement, you have understood the decision. If you cannot, you do not yet understand it clearly enough to record. The scaffold places a Y-statement at the top of every full ADR for this reason.
...
Superseded ADRs are never deleted. They move from `adr/accepted/` to `adr/superseded/` but remain in the record. This is because:
...
### Anti-patterns as first-class records

The scaffold treats anti-patterns — patterns that look reasonable but fail in practice — as first-class records with their own numbering sequence (`ap-NNN`). An anti-pattern can be:

- **Embedded** in the ADR whose decision surfaced it.
- **Promoted** to a standalone record when it becomes reusable across future decisions.

Promotion criteria are in `decision-process.md`. The point is that recognizing a bad pattern is valuable knowledge, and losing that knowledge is as costly as losing the knowledge of why good decisions were good.
...
- **MADR is the default template.** MADR 4.0-shaped, with Y-statement header prepended. See `../../adr/_templates/madr-full.md`.
- **Y-statements are required for full MADRs** to force clarity and enable quick-scan reading.
...
- **Granularity.** How fine-grained should an ADR be? One decision per ADR is the received wisdom, but "one decision" is fuzzy. The scaffold's rule: one Y-statement per ADR. If you need two Y-statements, you need two ADRs.
...
- Zimmermann, O. Y-statements essay. https://medium.com/olzzio/y-statements-10eb07b5a177.
