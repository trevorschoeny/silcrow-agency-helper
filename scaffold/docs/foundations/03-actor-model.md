...
|---|---|
| Actor | Agent (`agents/{role}/`) |
| Private state | Agent's own directory |
| Mailbox | `agents/{role}/inbox/` |
| Message history | `agents/{role}/inbox/archive/` |
| Asynchronous message | File deposit into another agent's inbox |
| Shared ledger | The ADR tree (`adr/accepted/`, `adr/superseded/`, `adr/rejected/`) |
